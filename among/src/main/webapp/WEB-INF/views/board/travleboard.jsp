        ...
        <article id = "postList">
        	<div id = "postList">
        		...
        		<section id = "generalList">
        			...
        			<c:choose>
        			<c:when test="${memGrade == 2 && board.boardCategory==4 && board.delYn eq 'N'}">
        			<a href="boardlist?boardKey=${board.boardKey}&category=${4}" class="notice">
        					...
        					<div class="info">
        						<div class="title">
        							...
        							<div class="etc">
        							<div id="nickName">
        								<img src="resources/images/travelAdminIcon.png">
        								<div>${board.nName}</div>
        							</div>
        				...
        				<c:set var = "memGrade" value="${board.memGrade}"/>
        				<c:choose>
        				<c:when test="${memGrade == 1 && board.boardCategory==4 && board.delYn eq 'N'}">
        		...
        		<div id="table">
				    ...
				    <li>
				        <a href='<c:url value="/travleboard?page=${pageMaker.startPage-1 }"/>'><i style = "list-decoration:none;">처음으로</i></a>
				    </li>
				    </c:if>
				    <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="pageNum">
				    <li>
				        <a href='<c:url value="/travleboard?page=${pageNum }"/>'><i class="fa">${pageNum }</i></a>
				    </li>
				    </c:forEach>
				    <c:if test="${pageMaker.next && pageMaker.endPage >0 }">
				    <li>
				        <a href='<c:url value="/travleboard?page=${pageMaker.endPage+1 }"/>'><i class="fa fa-chevron-right"></i></a>
				    </li>
				    </c:if>
        		</div>
