            		...
            		<div class = "pmypage1" style="height:900px;">
            				<div class = "pmypage2" style="width:1000px;">
                                	...
                                	<div class="Htable" aria-controls="loinid">
                                		...
                                		<c:choose>
                                		<c:when test = "${rep.boardCategory eq 4 && rep.delYn eq 'N'}">
	                                    <div class="Htext">
                                ...
                                <c:if test="${pageMaker.prev }">
							                <li>
							                    <a href='<c:url value="/myreply5?memKey=${param.memKey}&page=${pageMaker.startPage-1 }"/>'><i style = "list-decoration:none;">처음으로</i></a>
							                </li>
							                </c:if>
							                <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="pageNum">
							                <li>
							                    <a href='<c:url value="/myreply5?memKey=${param.memKey}&page=${pageNum }"/>'><i class="fa">${pageNum }</i></a>
							                </li>
							                </c:forEach>
							                <c:if test="${pageMaker.next && pageMaker.endPage >0 }">
							                <li>
							                    <a href='<c:url value="/myreply5?memKey=${param.memKey}&page=${pageMaker.endPage+1 }"/>'><i class="fa fa-chevron-right"></i></a>
							                </li>
							           </c:if>
							           </div>          
                            </div>
