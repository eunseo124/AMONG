            		...
            		<div class = "pmypage1" style="height:800px;">
            				<div class = "pmypage2" style="width:1000px;">
                                ...
                                <c:if test="${pageMaker.prev }">
							                <li>
							                    <a href='<c:url value="/mypost3?memKey=${param.memKey}&page=${pageMaker.startPage-1 }"/>'><i style = "list-decoration:none;">처음으로</i></a>
							                </li>
							                </c:if>
							                <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="pageNum">
							                <li>
							                    <a href='<c:url value="/mypost3?memKey=${param.memKey}&page=${pageNum }"/>'><i class="fa">${pageNum }</i></a>
							                </li>
							                </c:forEach>
							                <c:if test="${pageMaker.next && pageMaker.endPage >0 }">
							                <li>
							                    <a href='<c:url value="/mypost3?memKey=$${param.memKey}&page=${pageMaker.endPage+1 }"/>'><i class="fa fa-chevron-right"></i></a>
							                </li>
							           </c:if>
							           </div>
                            </div>
